...
public final class MDivTrans extends BinaryTrans {
    public MDivTrans(TransformerArgument argument, MDivExpr mDivExpr) {
        super(argument, mDivExpr);
    }
    ...
    public Pair<Expr, List<Stmt>> copyAndTransform() {
        if (this.hasTransformOnCurrentNode()) {
            ...
            AssignStmt t2Assign = buildAssignStmt.apply(
                    ...
                    new MDivExpr(new NameExpr(new Name(t0Name)), new NameExpr(new Name(t1Name)))
            );
            ...
            Expr copiedLHS = lhsTransformResult.getValue0();
            Expr copiedRHS = rhsTransformResult.getValue0();
            MDivExpr copiedNode = (MDivExpr) this.originalNode.copy();
